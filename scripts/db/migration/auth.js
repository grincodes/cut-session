...
const authInit = ()=>{
    const connection = mysql.createConnection({
      ...
      connection.connect((err) => {
        if (err) throw err;
        connection.query(
          `CREATE TABLE ${Tables.AUTH} ( authId varchar(255),username varchar(255) unique NOT NULL, password varchar(255) NOT NULL, accessType Enum ('MERCHANT', 'USER') NOT NULL, PRIMARY KEY (authId )  )`,
          (err, result) => {
